...
- Панель (`desktopUI: rhpane`, `mobileUI: fullscreen` — лист стягивается в половинное
  положение, карта остаётся видна; см. docs → plugin-layouts) для точки на карте:
...
- **Горизонт панели — 3 дня**, полная неделя остаётся на spearo.app: полоска из трёх
  чипов (короткий день недели + дата из `days[i].date`) и один чип-тизер «+4 дня на
  spearo →» — ссылка на прогноз того же города с кампанией `windy-plugin-days`.
  Дней, которых нет в ответе, в полоске просто нет — даты не выдумываются.
  Видимость — первая строка панели.
