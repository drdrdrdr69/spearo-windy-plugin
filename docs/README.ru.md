...
- **Горизонт 7 дней**: компактная полоска дней с горизонтальным скроллом (короткий день
  недели + дата из `days[i].date`); дни, которых нет в ответе, отключены — даты не
  выдумываются. Видимость — первая строка панели.
...
- `days=7` запрашивается одним запросом, «сырой» ответ кешируется по точке+локали
