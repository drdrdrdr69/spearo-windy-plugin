...
// dev-сборка = `npm start` (rollup -w с сервером). `npm run build` ставит SERVE=false.
// Флаг подставляется в код как литерал: в опубликованном бандле ветка подмены
// базового URL (?spearoBase=) вырезается компилятором.
const isDevBuild = process.env.SERVE !== 'false';
            ...
            jsc: {
                transform: {
                    optimizer: {
                        globals: {
                            vars: {
                                __SPEARO_DEV_BUILD__: isDevBuild ? 'true' : 'false',
                            },
                        },
                    },
                },
